...
for ge in ordfiles:#limitar el .dir
   if ge[-4:]=='orde' and ge[1:]!='.':
       os.system('uclust --input gpord/'+ge+' --uc gpalel/'+ge[:-5]+'.alel --id 0.95')
...
tabla = np.zeros([len(aislamientos)+1,len(alelcop)+1])
#tabla = tabla-1
tabla= list(tabla)
...
#--------------------------
prueba=0
#print(listagenes, str(len(listagenes)))
#print('    ..............       ')
#print(alelcop, str(len(alelcop)))
print('tamano alelecop '+str(len(alelcop)))
for gen in range(1,len(listagenes)):# no se le + 1 porque a lista genes se le agrego la etiqueta aislamientos
